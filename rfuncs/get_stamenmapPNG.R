get_stamenmapPNG <- function (bbox = c(left = -95.80204, bottom = 29.38048, right = -94.92313, 
  ...
  force = FALSE, where = tempdir(), ...) 
{
  ...
  listOfTiles <- lapply(split(tilesNeeded, 1:nrow(tilesNeeded)), 
    function(v) {
      v <- as.numeric(v)
      get_stamenmap_tile(maptype, zoom, v[1], v[2], force = force, color = "bw", 
        messaging = messaging)
    })
