...
lsGW <- list.files(path = "input/groundwater_updated/", full.names = TRUE)
lsGWna <- list.files(path = "input/groundwater_updated/") %>% 
    gsub("\\..*","", .) %>% 
    gsub('[[:digit:]]+', '', .) %>% 
    trimws(., "left")
...
source("rfuncs/read_gwdata_fin.R")
# gw.df <- lapply(lsGW, function (lsGW) {
#     return(tryCatch(read_gwdata_fin(lsGW), error=function(e) NULL))
# }
# )
gw.df <- lapply(lsGW, function(x){
  return(tryCatch(read_metadata_fin(x), error=function(e) NULL))
  }
  )
# gw.df <- Filter(function(x) !is.null(x), gw_srt)
# gw.df <- bind_rows(test)
...
gw.df.all <- #lapply(gw.df.all, mutate_if, is.numeric, as.character) %>% 
  ...
  bind_rows(., .id = "Stationname") %>%
    rename(
        ...
        top_OW = "Putken yläpää [m(N2000)]",
        bot_OW = "Putken alapää [m(N2000)]"
        ) %>%
